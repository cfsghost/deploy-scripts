...
```bash
./gcs.sh create                     # 建立 bucket（預設名稱: <專案ID>-uploads）
./gcs.sh grant my-backend           # 授權後端服務的執行身分讀寫
```
...
```bash
./gcs.sh cors https://app.example.com              # 多個來源用逗號分隔
```
...
|---|---|---|
| 預設名稱 | `<專案ID>-uploads` | `<專案ID>-public` |
| 放什麼 | 使用者上傳、隱私內容 | 網站靜態資源、公開下載檔 |
| 對外取用 | 後端簽發 presigned URL | 直接以 URL 存取（可掛網域 + CDN） |

```bash
./gcs.sh create --public                # 建立公開 bucket（會顯示整桶公開的警告）
./gcs.sh grant my-backend <專案ID>-public   # 後端也要寫這桶的話
```
...
# 或掛在既有網域的一段路徑下：
./lb.sh add_rule app.example.com '/static/*' --bucket <專案ID>-public
```
...
./gcs.sh create --public [bucket名稱]   建立【公開讀取】的 bucket（預設名稱 <專案ID>-public）
...
| `-b, --bucket <名稱>` | 指定 bucket 名稱（預設 `<專案ID>-uploads`；`--public` 時 `<專案ID>-public`） |
...
- **bucket 名稱被占用**：bucket 名稱是**全球唯一**的，預設的 `<專案ID>-uploads` 幾乎不會撞名；自訂名稱撞名時換一個即可。
