...
> **服務還沒部署？** `grant` / `create_hmac` 用服務名稱時需要該服務已完成第一次部署（階段 5）。想在部署前就把權限與金鑰準備好——例如讓首次部署就帶到 storage 環境變數——服務名稱換成 `default` 即可，工具會自動授權**專案預設 compute SA**（走本流程部署的服務用的就是這個身分）：
>
> ```bash
> ./gcs.sh grant default my-app-uploads
> ./gcs.sh create_hmac default my-app-uploads > .env.storage
> ```
